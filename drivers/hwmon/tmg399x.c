...
#define	AMS_ALS_POLL_DELAY_FAST	         msecs_to_jiffies(150)
#define	AMS_ALS_POLL_DELAY_SLOW	         msecs_to_jiffies(1000)
...
static int tmg399x_read_prox_data(struct tmg399x_chip *chip)
{
    ...
    {      
        ...
        SENSOR_LOG_INFO("TMG399X_GES_CFG_4 = 0x%X",temp);
        temp &= 0xFE;
...
static int tmg399x_ges_enable(struct tmg399x_chip *chip, int on)
{
	...
	if (on) 
    {
        ...
        ams_als_poll_time = AMS_ALS_POLL_DELAY_SLOW;
        tmg399x_als_atime_set(AMS_ALS_ATIME_SHORT);

        chip->ges_enabled = true;
    ...
    else 
    {
        ...
        {                        
            ams_als_poll_time = AMS_ALS_POLL_DELAY_FAST;
...
static int tmg399x_als_enable(struct tmg399x_chip *chip, int on)
{
	...
	if (on) 
    {
		...
		if (ret < 0)
			return ret;
		mdelay(3);
        schedule_delayed_work(&p_global_tmg399x_chip->als_poll_work, msecs_to_jiffies(ams_als_poll_time));
	}
    else
    {
        chip->als_enabled = false;
				...
				const char *buf, size_t size)
{
    ...
    input_report_rel(chip->p_idev, REL_RX,      Gesture_Result.gesture_style);
    input_report_rel(chip->p_idev, REL_RY,      Gesture_Result.enter_time);
    input_report_rel(chip->p_idev, REL_RZ,      Gesture_Result.exit_time);
    input_report_rel(chip->p_idev, REL_HWHEEL,  Gesture_Result.enter_angle);
    input_report_rel(chip->p_idev, REL_DIAL,    Gesture_Result.exit_angle);
    input_sync(chip->p_idev);
...
{   
    ...
    {
            //SENSOR_LOG_INFO("IRQ GES enter\n");
            atomic_set(&gesture_drdy, 1);
            wake_up(&gesture_drdy_wq);
            //SENSOR_LOG_INFO("IRQ GES exit\n");
    }
...
static void tmg399x_als_poll_work_func(struct work_struct *work)
{
    ...
    if (true == p_global_tmg399x_chip->ges_enabled)
    {
...
static int __devinit tmg399x_probe(struct i2c_client *client, const struct i2c_device_id *idp)
{
	...
	input_set_abs_params(chip->p_idev, ABS_X,        0, 1, 0, 0);
	input_set_abs_params(chip->p_idev, ABS_Y,        0, 1, 0, 0);
    */
...
bypass_als_idev:
    ...
    /*
    chip->prox_calibrate_times = 20;
    schedule_delayed_work(&p_global_tmg399x_chip->prox_calibrate_work, 1000);
    */
