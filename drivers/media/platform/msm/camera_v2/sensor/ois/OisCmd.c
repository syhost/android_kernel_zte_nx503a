...
// Measure Mode
 ...
 #define		BIAS_ADJ_BORDER	0x0CCC							// HALL_MAX_GAP < BIAS_ADJ_BORDER < HALL_MIN_GAP

 #define		HALL_MAX_GAP	BIAS_ADJ_BORDER - MARJIN
 #define		HALL_MIN_GAP	BIAS_ADJ_BORDER + MARJIN
 #define		BIAS_LIMIT		0x8000							// HALL BIAS LIMIT
...
// #define		MINLMT		0x40000000				// 2.0
 #define		MINLMT		0x3FC00000				// 1.5
// #define		CHGCOEF		0xBA1AAD55
 #define		CHGCOEF		0xB9F55555
 
 #define		MINLMT_MOV	0x00000000				// 0.0
 #define		CHGCOEF_MOV	0xB96AB000
#endif
...
{
	if( UcSonOff ) {
		RegWriteA( SWFC1, 0x69 ) ;								// 0x00DD		170.9Hz
...
//	RegWriteA( GDLYMON21, 0x01 ) ;		// 0x0187 <- GYADZ(0x18F5)
	...
	StAdjPar.StGvcOff.UsGxoVal = (unsigned short)GenMes( GYRMON1, 0 );		// 64回の平均値測定	GYRMON1(0x1110) <- GXADZ(0x19F5)
	...
	//////////
	BsyWit( DLYCLR2, 0x80 ) ;			// 0x00EF	Measure Filter1 Delay RAM Clear
	StAdjPar.StGvcOff.UsGyoVal = (unsigned short)GenMes( GYRMON2, 0 );		// 64回の平均値測定	GYRMON2(0x1111) <- GYADZ(0x18F5)
...
/* 振幅はwavxg(13C3h),wavyg(13C4h)で調整 */
void	SetSinWavePara( unsigned char UcTableVal ,	unsigned char UcMethodVal )
{
	...
	if( UcFreqDat == 0xF0 )			/* Sine波中止 */
	{
		...
		RegWriteA( MSF1EN, 0x00 ) ;				/* 0x00C0	測定用Filter1の演算OFF */
		RegWriteA( MSF2EN, 0x00 ) ;				/* 0x00C4	測定用Filter2の演算OFF */
	}
	else
	{
		RegWriteA( SINXADD, 0x00 ) ;			/* 0x00E3	---- */
		...
		RegWriteA( MSF1EN, 0x81 ) ;				/* 0x00C0	測定用Filter1の演算ON */
		RegWriteA( MSF2EN, 0x81 ) ;				/* 0x00C4	測定用Filter2の演算ON */
...
//	  RegWriteA( SWEN , 0x08 ); 												  // SWEN	  [ SINON | SINRST | - | - ][ SININISET | - | - | - ]
	...
	/* Set Frequency */
	//****************************************************
	//	周波数 = (fs/96)*(SWB+1)/(SWA+1)*1/(2^SWC)	[ Hz ]
	//****************************************************
...
void StartSineWave( void )
{
	/* Start Sine Wave */
	//****************************************************
	//	＊注意＊　サイン波を実行する前に以下のRAM/REGを設定してください　
	//				- RAM:1290h～12A8h (サイン波テーブル)
	//				- RAM:13C3h～13C4h (振幅)
	//				- RAM:11D5h～11D6h (開始座標)
	//				- REG:00DDh,00DEh  (周波数)
	//****************************************************
...
unsigned char	DrvPwmSw( unsigned char UcSelPwmMod )
{
	...
	switch ( UcSelPwmMod ) {
		case Mlnp :
			RegWriteA( LXEQFC2 	, 0x01 ) ;						// 0x0083		Linear補正ON
...
unsigned char	TneHvc( void )
{
	...
	RegWriteA( MS1INADD, 0x01 ) ;		// 0x00C2		HXIDATを測定する
	RegWriteA( MS2INADD, 0x04 ) ;		// 0x00C6		HYIDATを測定する
...
void	SetH1cMod( unsigned char	UcSetNum )
{
	...
	case( MOVMODE ):			// Movie mode 
		...
		RamWrite32A( gxl2a_2, MINLMT_MOV ) ;		/* 0x1860 L-Limit */
		RamWrite32A( gyl2a_2, MINLMT_MOV ) ;		/* 0x1960 L-Limit */

		RamWrite32A( gxl2a_2, MINLMT_MOV ) ;	/* 0x1860 L-Limit */
		RamWrite32A( gyl2a_2, MINLMT_MOV ) ;	/* 0x1960 L-Limit */

		RamWrite32A( gxl2c_2, CHGCOEF_MOV ) ;	/* 0x1862 Change coefficient gain */
		RamWrite32A( gyl2c_2, CHGCOEF_MOV ) ;	/* 0x1962 Change coefficient gain */
