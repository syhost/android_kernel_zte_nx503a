...
{
	static unchar ANX7808BA_FW_VER = 0x27;
...
unchar sp_tx_hw_link_training(void)
{
	...
	if (sp_tx_hw_lt_done) {
		...
		if ((bytebuf[0]&0x07) != 0x07) {
			...
			if (!sp_tx_test_lt) {
				sp_write_reg(TX_P0, SP_TX_LT_SET_REG, 0x0A);//set 6mv3.5db
		...
		case 0x02:
			if (AUX_OK == sp_tx_aux_dpcdread_bytes
				...
				if ((SINK_OUI[0] == 0xb9)
					...
					printk("sp_tx_aux_dpcdread_bytes AUX OK.,c=%d.\n",c);
					if((c&0x7f)>=0x15){
						bcharging__status_capable = 1;
		...
		case 0x02:
			if (AUX_OK == sp_tx_aux_dpcdread_bytes
				...
				if ((SINK_OUI[0] == 0xb9)
					...
					&& (SINK_OUI[6] == 0x00)) {
					sp_tx_rx_type = RX_HDMI;
					/*DEV_DBG("Downstream is\
7730 HDMI dongle.\n");*/

				} else {
					sp_tx_rx_type = RX_DP;
					/*DEV_DBG("sink is general\
DP2HDMI converter.\n");*/
				}
				ds_port_recoginze = 1;
			} else
...
static void sp_tx_irq_isr(void)
{
	...
	unchar Int_vector[2];
	unchar test_vector;
printk("sp_tx_irq_isr...begin...1.\n");
	if (AUX_OK == sp_tx_aux_dpcdread_bytes
		(0x00, 0x02, DPCD_SERVICE_IRQ_VECTOR, 1, bytebuf)) {
printk("sp_tx_irq_isr...begin...2.\n");
		IRQ_Vector = bytebuf[0];
		sp_tx_aux_dpcdwrite_bytes
			(0x00, 0x02, DPCD_SERVICE_IRQ_VECTOR, 1, bytebuf);
	} else {
printk("sp_tx_irq_isr...begin...3.\n");	
		return;
	}

#ifdef CONFIG_SLIMPORT_FAST_CHARGE
	/*charging irq*/
	printk("fast charge IRQ_Vector=%d, %d.\n", IRQ_Vector, bcharging__status_capable);
...
#ifdef CONFIG_SLIMPORT_CEC
			DEV_NOTICE("Receive CEC command from downstream done!\n");
			downstream_hdmi_cec_readmessage();
#endif
		}

		if ((Int_vector[0] & 0x80) == 0x80)
			DEV_NOTICE("CEC command transfer to downstream done!");
...
{
	DEV_NOTICE("**HDMI RX received CEC***");
	if ((sp_tx_rx_type == RX_HDMI)
